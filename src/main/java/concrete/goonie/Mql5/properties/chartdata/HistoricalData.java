...
public class HistoricalData {
     ...
     */
//    public void copyHistoricalDataAligned(Symbol symbol, HistoricalData source, HistoricalData target, ENUM_TIMEFRAME currentTimeFrame, int index) {
//        BarSeries currentSeries = source.getBarSeries(symbol, currentTimeFrame);
//        if (index < 0 || index >= currentSeries.size()) return;
//
//        long targetTimestamp = currentSeries.iTime(index); // The timestamp we are aligning to
//
//        // Iterate through all timeframes and copy bars matching this timestamp
//        for (ENUM_TIMEFRAME timeframe : source.getData().get(symbol).keySet()) {
//            BarSeries sourceSeries = source.getBarSeries(symbol, timeframe);
//            BarSeries targetSeries = target.getBarSeries(symbol, timeframe);
//
//            int matchingIndex = sourceSeries.iBarShift(targetTimestamp, true); // Find index with the same timestamp
//            if (matchingIndex >= 0) {
//                targetSeries.addCandle(sourceSeries.getBar(matchingIndex)); // Copy only matching bars
//            }
//        }
//    }
            ...
            for (ENUM_TIMEFRAME timeframe : ENUM_TIMEFRAME.values()) {
                ...
                if (lowestTimeframe == timeframe) {
                    ...
                    if (isNewTimeframe) {
                        ...
                        //  System.out.println("Created new " + timeframe + " bar at " + barStartTime);
                    } else {
                        ...
                        ///System.out.println("Updated " + timeframe + " bar at " + lastBar.getDateTime());
                    }
                }
